...
function SFGovTranslate() {
  ...
  this.sfgovGTranslate = function (event) {
    ...
    if (drupalTranslation) {
      ...
      // no drupal translation exists, use gtranslate
  ...
  this.setDrupalTranslationUrls = function () {
    ...
    if (drupalTranslations) {
      for (var i = 0; i < drupalTranslations.length; i++) {
        ...
        $('.gtranslate-link[data-sfgov-translate$="|' + translation.lang + '"]').attr('href', translation.turl).attr('data-sfgov-translator', translation.status === true ? 'drupal' : 'gtranslate');
  ...
  this.checkCurrentLanguage = function () {
    ...
    var gTranslateLang = gTranslateCookie ? gTranslateCookie.split('/')[2] : null; // If translation cookie lang is different from Drupal current language,
    ...
    if (currentDrupalLanguage != 'en' || currentDrupalLanguage != gTranslateLang) {
      var drupalTranslation = that.getDrupalTranslation(currentDrupalLanguage); // Always translate page, even if a Drupal translation for the page exists.
    ...
    if (gTranslateLang && gTranslateLang != 'en') {
      // gtranslate cookie exists, a page was gtranslated somewhere
      that.addElementTranslationClass(gTranslateLang);
      var drupalTranslation = that.getDrupalTranslation(gTranslateLang);

      if (drupalTranslation && drupalTranslation.turl != window.location.pathname) {
        // drupal translation exists
        $.when(that.sfgovDoGTranslate('en|en')).then(function () {
          // kill the cookie and redirect to the drupal translation
          window.location.href = drupalTranslation.turl;
        });
      }
    }
...
(function ($) {
  ...
  var callback = function callback(mutationsList, observer) {
    ...
    if (elem) {
      // Attach click event.
      $(elem).on('click', t.sfgovGTranslate); // this dropdown list that gets added doesn't always have the english option
