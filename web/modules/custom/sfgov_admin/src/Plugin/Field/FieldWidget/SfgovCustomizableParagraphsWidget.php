...
class SfgovCustomizableParagraphsWidget extends InlineParagraphsWidget {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($handler_settings['target_bundles'] as $bundle) {
      ...
      $element['paragraph_settings'][$bundle]['disabled_new'] = [
        '#title' => $this->t('Disable this paragraph type for new content'),
        '#type' => 'checkbox',
        '#description' => $this->t('If checked, this paragraph type will not be available for new content.'),
        '#default_value' => (bool) $paragraph_settings[$bundle]['disabled_new'],
      ];
  ...
  protected function getAccessibleOptions() {
    $options = parent::getAccessibleOptions();
    $paragraph_settings = $this->getSetting('paragraph_settings');
    $disabled_options = array_filter($paragraph_settings, function ($settings) {
      return (bool) $settings['disabled_new'];
    });
    return array_diff_key($options, $disabled_options);
  }

  protected function buildAddActions() {
    return parent::buildAddActions(); // TODO: Change the autogenerated stub
  }
