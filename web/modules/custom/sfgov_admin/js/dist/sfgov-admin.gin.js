...
(function ($, Drupal, document, window) {
  ...
  Drupal.behaviors.sfgovAdminSidebarToggle = {
    attach: function (context) {
      $('.layout-region-node-secondary', context).once('sidebarToggle').wrapInner('<div id="sidebar-toggle-content"/>').prepend('<button id="sidebar-toggle" class="sidebar-toggle"><span class="sidebar-toggle__text">' + Drupal.t('Toggle sidebar') + '</span><span class="sidebar-toggle__icon sidebar-toggle-x"></span></button>');
      $('#sidebar-toggle', context).once().on('click', function (event) {
        event.preventDefault();
        var $body = $('.gin--edit-form', context);
        var $sidebar = $('#sidebar-toggle-content', context);
        var $icon = $('.sidebar-toggle__icon', context);

        if ($body.hasClass('sidebar-toggle-active')) {
          $sidebar.show();
          $body.removeClass('sidebar-toggle-active');
          $icon.removeClass('sidebar-toggle-x');
          $icon.addClass('sidebar-toggle-hamburger');
        } else {
          $sidebar.hide();
          $body.addClass('sidebar-toggle-active');
          $icon.addClass('sidebar-toggle-x');
          $icon.removeClass('sidebar-toggle-hamburger');
        }
      });
    }
  };
...
//# sourceMappingURL=sfgov-admin.gin.js.map
